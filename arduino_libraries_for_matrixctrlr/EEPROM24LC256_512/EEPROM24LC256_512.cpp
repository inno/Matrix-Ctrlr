...
void EEPROM256_512::writeByte(unsigned int addr, unsigned char data)
{
...
void EEPROM256_512::readPage(unsigned int page_number, unsigned char* data)
{
...
void EEPROM256_512::writePage(unsigned int page_number, unsigned char* data)
{
...
void EEPROM256_512::writePageFast(unsigned int page_number, unsigned char* data)
{
	//assuming the data* is size of a page
    uint8_t index = 0;
    unsigned int lower_addr=0;
    ...
    for(itr=0;itr<((page_size)/(MINIBUFFER)); itr++)
    {
		...
		writeMiniBufferFast(lower_addr);
	}//end itr loop
}//end writePage
...
void EEPROM256_512::writeMiniBufferFast(unsigned int addr)
{
	uint8_t ct=0;
    Wire.beginTransmission(fulladdr);//begin transmission on wire
    //32768 bytes... addressed by 2 bytes (16bits)
